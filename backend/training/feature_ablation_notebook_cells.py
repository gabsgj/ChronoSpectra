...
def _feature_ablation_output_dir_source() -> str:
    return """
from pathlib import Path

OUTPUT_DIR = Path("chronospectra_feature_ablation_artifacts")
OUTPUT_DIR.mkdir(exist_ok=True)
REPORTS_DIR = OUTPUT_DIR / "reports"
REPORTS_DIR.mkdir(exist_ok=True)

print("Feature ablation output dir:", OUTPUT_DIR)
    """


def _run_feature_ablation_source(mode: str) -> str:
    return dedent(
        f"""
...
def write_feature_ablation_report(stock_id: str, mode_name: str, entries: list[dict]) -> dict:
    ...
    report_path = REPORTS_DIR / f"{{stock_id}}_{{mode_name}}_feature_ablation_report.json"
    payload = {{
        "stock_id": stock_id,
        "mode": mode_name,
...
def run_per_stock_feature_ablation() -> list[dict]:
    ...
    for stock_index, stock_config in enumerate(ACTIVE_STOCKS):
        ...
        for label, channels, removed_channel in build_channel_sets():
            ...
            checkpoint_path = OUTPUT_DIR / f"{{stock_config['id']}}_{{NOTEBOOK_MODE}}_{{label}}.pth"
...
def run_unified_feature_ablation(mode_name: str) -> list[dict]:
    ...
    for label, channels, removed_channel in build_channel_sets():
        ...
        checkpoint_path = OUTPUT_DIR / f"{{mode_name}}_{{label}}.pth"
...
aggregate_report_path = OUTPUT_DIR / "feature_ablation_report.json"
