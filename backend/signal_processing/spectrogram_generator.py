...
class SpectrogramGenerator:
    ...
    def generate(
        ...
        spectrogram, frequency_axis, time_axis = transform.transform(feature_signal.normalized_values)
        time_timestamps = self._map_time_axis_to_timestamps(time_axis, feature_signal.timestamps)
        png_bytes = self._render_png(
            ...
            resolved_transform,
            feature_signal.feature_channel,
        )
        return SpectrogramArtifact(
            ...
            raw_signal=feature_signal.raw_values.astype(float).tolist(),
    ...
    def generate_stft_frames(
        ...
        frames, frequency_axis = transform.build_frames(
            feature_signal.raw_values,
            feature_signal.normalized_values,
            feature_signal.timestamps,
        )
    ...
    def _render_png(
        ...
        axis.set_title(
            f"{display_name} {feature_channel.upper()} {transform_name.upper()} Spectrogram"
        )
